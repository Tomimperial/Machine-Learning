...
table(mydat2$income)
#SECTION 9
# We improvise on sampling techniques to improve our model fit
# We rename the data to avoid R confusion in naming i.e marital-status is replaced as marital
# _status
colnames(mydat2)<-c("age" ,           "workclass" ,     "education_num",
                    "marital_status", "occupation" ,  "relationship" ,
                    "race"  ,         "sex"     ,       "capital_gain",
                    "capital_loss" ,  "hours_per_week", "native_country" ,"income" )
# Resampling with both method - over and under
data.bal <- ovun.sample(income~., data=mydat2,
                        N=nrow(mydat2), p=0.5, method="both")$data
# Checking the class proportions
table(data.bal$income)
# Converting new data to the h2o frame 
df1<-as.h2o(data.bal)
# Spliting train test and validation
splits1 <- h2o.splitFrame(
  df1,           ##  splitting the H2O frame we read above
  c(0.6,0.2),   ##  create splits of 60% and 20%; 
  seed=1234)

train1 <- h2o.assign(splits1[[1]], "train.hex")   
valid1 <- h2o.assign(splits1[[2]], "valid.hex") 
test1 <- h2o.assign(splits1[[3]], "test.hex") 

# Training GBM with new data
cov_gbm <- h2o.gbm(x = 1:12, y = 13, 
                   training_frame = train1,
                   validation_frame = valid1, 
                   model_id="dl_model2_gbm",
                   seed = 1234)
# Checking performance with test data
gbm_test2_perf<-h2o.performance(h2o.getModel('dl_model2_gbm'), newdata = test)

# Resampling with over method
data.bal <- ovun.sample(income~., data=mydat2,
                        p=0.5, method="over")$data
# checking class proportions
table(data.bal$income)
#converting new data to h2o frame
df1<-as.h2o(data.bal)
...
splits1 <- h2o.splitFrame(
  ...
  seed=1234)
train1 <- h2o.assign(splits1[[1]], "train.hex")   
valid1 <- h2o.assign(splits1[[2]], "valid.hex")   ## R valid, H2O valid.hex
test1 <- h2o.assign(splits1[[3]], "test.hex") 
# Training GBM with new train data
cov_gbm <- h2o.gbm(x = 1:12, y = 13, 
                   training_frame = train1,
                   validation_frame = valid1, 
                   model_id="dl_model2_gbm",
                   seed = 1234)
# Checking model performance with old data
gbm_test2_perf<-h2o.performance(h2o.getModel('dl_model2_gbm'), newdata = test)

# Resampling with under method
data.bal <- ovun.sample(income~., data=mydat2,
...
cov_gbm <- h2o.gbm(x = 1:12, y = 13, 
                   ...
                   model_id="dl_model2_gbm",
                   seed = 1234)
# Checking performance of the data with same test 
gbm_test2_perf<-h2o.performance(h2o.getModel('dl_model2_gbm'), newdata = test)
